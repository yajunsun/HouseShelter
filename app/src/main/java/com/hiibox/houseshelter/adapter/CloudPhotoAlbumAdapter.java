...
public class CloudPhotoAlbumAdapter extends BaseAdapter {
	...
	private List<List<String>> picIdTimeList = null;

	private FinalBitmap finalBitmap = null;
	private String unit = null;
	private TreeMap<String, List<PhotoInfo>> mMap = null;

	public CloudPhotoAlbumAdapter(Context context, Activity activity,
		...
		picIdTimeList = new ArrayList<List<String>>();
	}
	...
	public void setMap(TreeMap<String, List<PhotoInfo>> map) {
		...
		for (Iterator<Map.Entry<String, List<PhotoInfo>>> it = set.iterator(); it
			...
			picIdTimeList.add(0, idtimeList);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final ArrayList<String> picL = (ArrayList<String>) picList
				.get(position);
		final ArrayList<String> picIdL = (ArrayList<String>) picIdList
				.get(position);
		final ArrayList<String> picTimeL = (ArrayList<String>) picIdTimeList
				.get(position);

		ViewHolder holder = new ViewHolder();
		if (null == convertView) {
			convertView = View.inflate(context,
					R.layout.lv_item_cloud_photo_album_layout, null);
		}
		holder.photoAlbumLayout = (LinearLayout) convertView
				.findViewById(R.id.photo_album_layout);
		holder.typeLayout = (LinearLayout) convertView
				.findViewById(R.id.type_layout);
		holder.row1Layout = (LinearLayout) convertView
				.findViewById(R.id.pic_row1_layout);
		holder.row2Layout = (LinearLayout) convertView
				.findViewById(R.id.pic_row2_layout);
		holder.photoNumbersTV = (TextView) convertView
				.findViewById(R.id.photo_numbers_tv);
		...
		holder.tempAbnormalTV = (TextView) convertView
				.findViewById(R.id.temperature_abnormal_tv);
		...
		case 0:
			...
			break;
		case 1:
			displayPic1(picL, picIdL, holder, picDate, picTimeL);
		...
		case 2:
			displayPic1(picL, picIdL, holder, picDate, picTimeL);
			displayPic2(picL, picIdL, holder, picDate, picTimeL);
			holder.pic3IV.setVisibility(View.GONE);
			holder.pic4IV.setVisibility(View.GONE);
			holder.row2Layout.setVisibility(View.GONE);
			break;
		case 3:
			displayPic1(picL, picIdL, holder, picDate, picTimeL);
			displayPic2(picL, picIdL, holder, picDate, picTimeL);
			displayPic3(picL, picIdL, holder, picDate, picTimeL);
		...
		default:
			...
			displayPic2(picL, picIdL, holder, picDate, picTimeL);
			displayPic3(picL, picIdL, holder, picDate, picTimeL);
			displayPic4(picL, picIdL, holder, picDate, picTimeL);
			break;
		}
	...
	private void displayPic4(final ArrayList<String> picL,
			final ArrayList<String> picIdL, ViewHolder holder,
			final String picDate, final ArrayList<String> picTime) {
		if (!TextUtils.isEmpty(picL.get(3))) {
			finalBitmap.display(holder.pic4IV, picL.get(3));
			holder.pic4IV.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					redirectTo(picL, picIdL, picDate,picTime);
				}
			});
		}
	}

	private void displayPic3(final ArrayList<String> picL,
			final ArrayList<String> picIdL, ViewHolder holder,
			final String picDate, final ArrayList<String> picTime) {
		if (!TextUtils.isEmpty(picL.get(2))) {
	...
	private void displayPic2(final ArrayList<String> picL,
			final ArrayList<String> picIdL, ViewHolder holder,
			final String picDate, final ArrayList<String> picTime) {
		if (!TextUtils.isEmpty(picL.get(1))) {
	...
	private void displayPic1(final ArrayList<String> picL,
			final ArrayList<String> picIdL, ViewHolder holder,
			final String picDate, final ArrayList<String> picTime) {
		if (!TextUtils.isEmpty(picL.get(0))) {
	...
	private void setLargeScreenParams(ViewHolder holder) {
		...
		if (screenHeight > 854 && screenHeight <= 1280) {
			...
			LayoutParams row2Params = new LayoutParams(
					LayoutParams.MATCH_PARENT, 200);
	...
	private void redirectTo(final ArrayList<String> picL,
			ArrayList<String> picIdL, final String picDate, ArrayList<String> picTime) {
		Intent intent = new Intent(context,
				CloudPhotoAlbumDetailsActivity.class);
		...
		intent.putExtra("picTime", picTime);
