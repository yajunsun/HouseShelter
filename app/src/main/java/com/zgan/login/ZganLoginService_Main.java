...
public class ZganLoginService_Main implements Runnable {
	...
	private Queue<Frame> Queue_Function = new LinkedList<Frame>();
	...
	public ZganLoginService_Main(Queue<byte[]> _queue, Queue<Frame> _fqueue) {
		Queue = _queue;
	...
	public void run() {
		...
		while (true) {
			try {
				...
				if (ZganLoginService_Listen.LoginServerState == 1
	...
	private boolean checkUserLogin(String strData) {
		if (!TextUtils.isEmpty(strData)) {
			...
			if (aryData.length == 2 && aryData[0].equals("0")) {
				return true;
			}
		}
